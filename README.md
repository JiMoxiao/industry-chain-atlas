...
该脚本自动完成依赖安装、数据生成、前后端后台启动，并打开浏览器访问 `http://localhost:5173/`。

> 首次运行若被 PowerShell 拦截，先执行 `Set-ExecutionPolicy -Scope Process Bypass`。
...
|------|------|
| 停止后台服务 | `.\stop.ps1` |
| 刷新全部研究数据 | `.\refresh-data.ps1` |
| 仅刷新热度缓存 | `.\refresh-data.ps1 -HeatOnly` |
...
> `refresh-data.ps1` 优先调用后端接口刷新；后端未启动时自动回退到本地脚本模式。
...
`.\start.ps1` 默认将前后端置于后台运行。确认方式：浏览器访问 `http://localhost:5173/`，或查看日志 `.dbg/runtime/logs/`。
